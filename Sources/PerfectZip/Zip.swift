        ...
        for path in processedPaths {
            ...
            if !thisFile.isDir {
                ...
                if password != "", let fileName = fileName {
                    ...
                    let filePath: String
                    if path.fileDir.count > 0 {
                        filePath = path.fileDir+"/"+fileName
                    } else {
                        filePath = fileName
                    }
                    
                    zipOpenNewFileInZip3(zip, filePath, &zipInfo, nil, 0, nil, 0, nil,Z_DEFLATED, Z_DEFAULT_COMPRESSION, 0, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, nil, 0)
                }
                else {
                    return ZipStatus.ZipFail
                }
